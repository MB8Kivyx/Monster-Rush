...
public class SplashFlowController : MonoBehaviour
{
    ...
    [Header("Next Scene")]
    public string nextSceneName = "MainMenu";  // UPDATED HERE
    ...
    private void Awake()
    {
        ...
        if (logoMark != null) logoMark.localScale = _markStartScale * 0.8f;
        if (logoMarkCg != null) logoMarkCg.alpha = 0f;
        if (logoTitle != null) logoTitle.localScale = Vector3.one * 0.95f;
        if (logoTitleCg != null) logoTitleCg.alpha = 0f;
        if (taglineCg != null) taglineCg.alpha = 0f;
    ...
    private IEnumerator ExitAndActivate(AsyncOperation op)
    {
        if (_loading || op == null) yield break;
